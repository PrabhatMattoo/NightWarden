...
- `commands/` inside apps/runner holds provider-agnostic dispatch plus host/file handlers; provider-specific execution lives in `apps/runner/src/docker/` and `apps/runner/src/kubernetes/`. LLM tool schemas (name, description, JSON schema, `access` gating) live in `apps/api/src/agent/tools.ts` as `TOOL_REGISTRY`; `packages/shared/src/tools.ts` holds only the tool input/output payload types shared between API and runner.
